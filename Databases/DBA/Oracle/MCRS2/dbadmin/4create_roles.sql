...
-----------
Script to Create roles and grant permissions.
----------
...
select '/u01/app/oracle/admin/scripts/logs_' vpath, SYS_CONTEXT ('USERENV', 'instance_name') loc, '/create_roles.log' ext from dual;  
...
drop role PS_MB_SIUD cascade
/ 
CREATE ROLE PS_MB_SIUD
/

drop role PS_MB_SIU cascade
/ 
CREATE ROLE PS_MB_SIU
/

drop role PS_MB_S cascade
/ 
CREATE ROLE PS_MB_S
/

BEGIN
FOR x IN (select * from dba_objects where owner = 'SYSADM' and object_type in ('TABLE','VIEW'))
LOOP
EXECUTE IMMEDIATE 'GRANT SELECT, INSERT, UPDATE, DELETE ON SYSADM.' || x.object_name || ' TO PS_MB_SIUD ';
EXECUTE IMMEDIATE 'GRANT SELECT, INSERT, UPDATE ON SYSADM.' || x.object_name || ' TO PS_MB_SIU ';
EXECUTE IMMEDIATE 'GRANT SELECT ON SYSADM.' || x.object_name || ' TO PS_MB_S ';
END LOOP;
END;
/
