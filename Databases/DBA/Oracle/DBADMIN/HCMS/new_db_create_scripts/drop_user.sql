--http://docs.oracle.com/cd/A87860_01/doc/appdev.817/a77069/10_dynam.htm
--can't use dynamic sql while dropping objects
SET ECHO OFF
...
--/
DECLARE
v_obj_to_drop varchar2(100);

BEGIN

FOR x IN (select * from dba_objects where owner = upper('&&user_to_drop') and object_type in ('INDEX'))
LOOP
v_obj_to_drop := x.object_name;
EXECUTE IMMEDIATE 'DROP INDEX ' || upper('&&user_to_drop') ||'.' || :v_obj_to_drop || '  ';
END LOOP;


FOR x IN (select * from dba_objects where owner = upper('&&user_to_drop') and object_type in ('TABLE'))
LOOP
v_obj_to_drop := x.object_name;
EXECUTE IMMEDIATE 'DROP TABLE ' || upper('&&user_to_drop') ||'.' || :v_obj_to_drop || ' CASCADE CONSTRAINTS ';
END LOOP;


FOR x IN (select * from dba_objects where owner = upper('&&user_to_drop') and object_type in ('VIEW'))
LOOP
v_obj_to_drop := x.object_name;
EXECUTE IMMEDIATE 'DROP VIEW ' || upper('&&user_to_drop') ||'.' || :v_obj_to_drop || ' CASCADE CONSTRAINTS ';
END LOOP;
