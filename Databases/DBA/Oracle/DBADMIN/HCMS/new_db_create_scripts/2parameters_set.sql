...
alter system set log_archive_format = 'HRTRN_%s_%t_%r.ARC' scope=spfile;

--Increase flash/fast recovery are
alter system set db_recovery_file_dest_size=19G SCOPE=BOTH;
...
-- This is to solve the issue of huge trace file generation. Refer to MOS doc 1380006.1 to findout more. *** Read the doc first
...
-- This is to solve the issue of huge trace file generation. If using ASM below parameter has no effect. With ASM, disk_async_io above is used. See Doc: 120697.1
alter system set filesystemio_options=none scope=spfile
alter system set filesystemio_options=asynch scope=spfile;

****** IMPORTANT ****** 
****** At this point do the following from doc ID 139272.1 to enable asynch io.
******
-- Also do as mentioned in below link to check the rate of transfer. when I have time.
...
-- Below parameter is to ensure that datapump doesn't fail with ORA-39079. Set this parameter if the memory target is 1G if more than 1G then no problem
-- even with out this parameter.
...
add the above parameter at the top of the cluvfy script in the <grid_infrastructure_home>/bin directory and this resolves the issue.

Logs are not under $ORACLE_BASE but under $ORACLE_HOME/cv/log of grid user. 

This is not observed in hseax09

--***********************************
--***********************************
--ASM Instance -- Below parameter you can set it via asmca, just add a new path with a "," in the GUI.
-- these parameter strings are correct. if Mark gives the disk as /dev/disk/*, there will be an equivalant /dev/rdisk/*. Always use rdisk for granting permissions on disks to grid.
