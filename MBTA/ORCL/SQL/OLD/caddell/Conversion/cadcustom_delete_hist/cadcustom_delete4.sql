...
Select count(1) into v_cnt from JBITEMNAMES;
delete 
from JBITEMNAMES;
...
Select count(1) into v_cnt from JBCONTDET;
delete 
from JBCONTDET;
 	v_cnt_err := v_cnt_err+1;
	INSERT into da1.errors(ers_order, ers_rownum, ers_table_name, ers_column_name) 
...
Select count(1) into v_cnt from JBCONT;
delete 
from JBCONT;
 	v_cnt_err := v_cnt_err+1;
	INSERT into da1.errors(ers_order, ers_rownum, ers_table_name, ers_column_name) 
...
Select count(1) into v_cnt from VOURLSDET;
delete 
from VOURLSDET;
 	v_cnt_err := v_cnt_err+1;
	INSERT into da1.errors(ers_order, ers_rownum, ers_table_name, ers_column_name) 
...
select count(1) into v_cnt
from batch 
where bch_num in ' || v_batch_del_str;
...
select count(1) into v_cnt
from jcdetail where jcdt_comp_code not in ('ZZ');
...
select count(1) into v_cnt from PYTAXEXM;
...
Truncate table da.scmast;
--delete from da.scmast
where SCMST_COMP_CODE not in ('ZZ');
 	v_cnt_err := v_cnt_err+1;
	INSERT into da1.errors(ers_order, ers_rownum, ers_table_name, ers_column_name) 
	values(v_cnt_err, v_cnt, v_tab_name,'DELETE COMPLETE');
...
select count(1) into v_cnt where bpven_bp_code not in ('ZZ-ACME','ZZ-HDEPO','ZZ-EANDL','ZZ-CGRP','ZZ-BCBS','ZZ-WMT');
...
select count(1) into v_cnt from pyemphist where emh_comp_code not in ('ZZ');
...
select count(1) into v_cnt pyemppayrate;
