...
v_time_jcat1    BINARY_INTEGER;
v_time_jcat2    BINARY_INTEGER;
v_time_aft_jcat BINARY_INTEGER;
TYPE CUR_REF IS  REF CURSOR;
cur_var          CUR_REF;
      v_batch_del_str   VARCHAR2(32767);
--      v_sqlstmt          VARCHAR2 (2048);
v_count1        NUMBER := 0;
v_total         NUMBER := 0;
v_csr_valu      ROWID;
...
from JBCONTDET;
	...
	values(v_cnt_err,v_tab_name,'DELETE COMPLETE');
commit;
...
execute immediate v_sqlstmt;
 	v_cnt_err := v_cnt_err+1;
	INSERT into da1.errors(ers_order, ers_table_name, ers_column_name) 
	values(v_cnt_err,v_tab_name,'DELETE COMPLETE for GLEDGER');
commit;
...
v_sqlstmt:= 'SELECT rowid from pyemppayhist where phy_comp_code not in (''ZZ'')';
OPEN cur_var FOR v_sqlstmt;
LOOP
FETCH cur_var INTO v_csr_valu;
        v_time_jcat1 := DBMS_UTILITY.GET_TIME;
DELETE FROM pyemppayhist
where phy_comp_code not in ('ZZ');
        v_time_jcat2 := DBMS_UTILITY.GET_TIME;
EXIT WHEN cur_var%NOTFOUND;
...
v_count1 := 0;
 	...
 	v_cnt_err := v_cnt_err+1;
	INSERT into da1.errors(ers_order, ers_table_name, ERS_ROWNUM, ers_column_name, ERS_ERR_TYPE) 
	values(v_cnt_err,'pyemppayhist', (v_time_jcat2-v_time_jcat1)/100, 'Seconds to delete', to_char(v_total));
COMMIT;
END IF;
END LOOP;
CLOSE cur_var;
...
v_time_bef_jcat := DBMS_UTILITY.GET_TIME;
v_sqlstmt:= 'SELECT rowid from jcjobcat where jcat_comp_code not in (''ZZ'')';
OPEN cur_var FOR v_sqlstmt;
LOOP
FETCH cur_var INTO v_csr_valu;
        v_time_jcat1 := DBMS_UTILITY.GET_TIME;
...
v_count1 := 0;
 	...
 	v_cnt_err := v_cnt_err+1;
	INSERT into da1.errors(ers_order, ers_table_name, ERS_ROWNUM, ers_column_name, ERS_ERR_TYPE) 
	values(v_cnt_err,'JCJOBCAT', (v_time_jcat2-v_time_jcat1)/100, 'Seconds to delete', to_char(v_total));
COMMIT;
END IF;
END LOOP;
CLOSE cur_var;
...
DBMS_OUTPUT.PUT_LINE ('DELETE, COMMIT ON ALL TABLES SUCCESSFUL.');
 	v_cnt_err := v_cnt_err+1;
	INSERT into da1.errors(ers_order, ers_table_name, ers_column_name) 
	values(v_cnt_err,'---ALL TABLES---','---DELETE COMPLETE---');
...
WHEN OTHERS 
THEN
--DBMS_OUTPUT.PUT_LINE ('TIME TAKEN TO DELETE TABLES IS: '||TO_CHAR((v_time_after - v_time_before)/100));
--dbms_output.put_line(chr(9));
