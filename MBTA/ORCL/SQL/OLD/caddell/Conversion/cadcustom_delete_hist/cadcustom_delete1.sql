...
dbms_output.put_line(chr(9));
DBMS_OUTPUT.PUT_LINE (v_time_after - v_time_before);
 	v_cnt_err := v_cnt_err+1;
	INSERT into da1.errors(ers_order, ERS_ROWNUM, ers_column_name) 
	values(v_cnt_err, v_time_after - v_time_before, '-TOT TIME FOR DEL ON ALL TABS-');
commit;
...
DBMS_OUTPUT.PUT_LINE('Error while deleting table: '||v_tab_name||'. ');
	v_err_strng:=SQLERRM;
        ...
        time_after := DBMS_UTILITY.GET_TIME;
 	v_cnt_err := v_cnt_err+1;
	INSERT into da1.errors(ers_order, ers_description, ERS_ROWNUM) 
	values(v_cnt_err, 'TOT TIME FOR DELETE ', v_time_after - v_time_before);
