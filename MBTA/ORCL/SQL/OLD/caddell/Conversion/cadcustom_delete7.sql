...
PROMPT
PROMPT
PROMPT CHECK DA1.ERRORS TABLE FOR TABLE BY TABLE DELETION STATUS, WHILE PROCESS IN PROGRESS.
...
v_tab_name :=upper('HREMRELATIVES');
delete 
from HREMRELATIVES;
...
v_tab_name :=upper('JCMCAT');
delete 
from JCMCAT
where phsc_comp_code <> 'ZZ';
 	v_cnt_err := v_cnt_err+1;      v_cnt:= SQL%ROWCOUNT;
	INSERT into da1.errors(ers_order, ers_rownum, ers_table_name, ers_column_name) 
	values(v_cnt_err, v_cnt, v_tab_name,'DELETE COMPLETE');
commit;
