...
--PYJOBALLOC_JCMPHS_FK

cursor cPYJOBALLOC_JCMPHS_FK is
  select dc_rownum,
	 PYJA_COMP_CODE,
	 PYJA_PHS_CODE
	from DA.DC_PYJOBALLOC  T1
	where not exists
           (select '1'
              from DA.JCMPHS T2
                where nvl(T1.PYJA_COMP_CODE,T2.PHS_COMP_CODE) = T2.PHS_COMP_CODE
		  and nvl(T1.PYJA_PHS_CODE,T2.PHS_CODE) = T2.PHS_CODE );
...
--PYJOBALLOC_JCMPHS_FK
 for row_dc in cPYJOBALLOC_JCMPHS_FK
 loop
  if ( row_dc.PYJA_COMP_CODE is not null 
	 and  row_dc.PYJA_PHS_CODE is not null  ) then
	 da.dbk_dc.error('DC_PYJOBALLOC',
                                 row_dc.dc_rownum,
                                 'PYJOBALLOC_JCMPHS_FK',
                                 'JCMPHS',
                'Record with '|| ' PHS_COMP_CODE '||row_dc.PYJA_COMP_CODE||
		','||' PHS_CODE '||row_dc.PYJA_PHS_CODE||
...
--SYS_C0095031 - "PYJA_CODE" IS NOT NULL
 cursor cur_SYS_C0095031 is
        select dc_rownum
          from DA.DC_PYJOBALLOC
            where not "PYJA_CODE" IS NOT NULL ;

--SYS_C0095030 - "PYJA_TYPE_CODE" IS NOT NULL
 cursor cur_SYS_C0095030 is
        select dc_rownum
          from DA.DC_PYJOBALLOC
            where not "PYJA_TYPE_CODE" IS NOT NULL ;

--SYS_C0095029 - "PYJA_JOB_CODE" IS NOT NULL
 cursor cur_SYS_C0095029 is
        select dc_rownum
          from DA.DC_PYJOBALLOC
            where not "PYJA_JOB_CODE" IS NOT NULL ;

--SYS_C0095028 - "PYJA_COMP_CODE" IS NOT NULL
 cursor cur_SYS_C0095028 is
        select dc_rownum
          from DA.DC_PYJOBALLOC
            where not "PYJA_COMP_CODE" IS NOT NULL ;

--PYJA_TYPE_CODE_CHK_BNDETXWCPL - pyja_type_code IN ('BN', 'DE', 'TX','WC','PL')
 cursor CUR_PYJA_TYPE_CODE_CHK_BNCPL is
        select dc_rownum
          from DA.DC_PYJOBALLOC
...
null;
 ...
 for row_dc in cur_SYS_C0095031
 loop
    da.dbk_dc.error('DC_PYJOBALLOC',
                    row_dc.dc_rownum,
                    'SYS_C0095031',
 ...
 for row_dc in cur_SYS_C0095030
 loop
    da.dbk_dc.error('DC_PYJOBALLOC',
                    row_dc.dc_rownum,
                    'SYS_C0095030',
 ...
 for row_dc in cur_SYS_C0095029
 loop
    da.dbk_dc.error('DC_PYJOBALLOC',
                    row_dc.dc_rownum,
                    'SYS_C0095029',
 ...
 for row_dc in cur_SYS_C0095028
 loop
    da.dbk_dc.error('DC_PYJOBALLOC',
                    row_dc.dc_rownum,
                    'SYS_C0095028',
 ...
 for row_dc in CUR_PYJA_TYPE_CODE_CHK_BNCPL
 loop
    da.dbk_dc.error('DC_PYJOBALLOC',
                    row_dc.dc_rownum,
  ...
  cursor cur_dc is
	...
	from DA.DC_PYJOBALLOC T1
	  where not exists (select '1'
                        from DA.JCJOBCAT  T2
                          where T1.PYJA_COMP_CODE = T2.JCAT_COMP_CODE
			    and T1.PYJA_JOB_CODE = T2.JCAT_JOB_CODE
			    and T1.PYJA_PHS_CODE = T2.JCAT_PHS_CODE
			    and T1.PYJA_CAT_CODE = T2.JCAT_CODE );

BEGIN
  for row_dc in cur_dc
  loop
    if ( row_dc.PYJA_PHS_CODE is not null ) then 
	 	da.dbk_dc.error('DC_PYJOBALLOC',
                row_dc.dc_rownum,
                'PYJA_PHS_CODE',
                'JCJOBCAT',
                'Record with'||
		' JCAT_COMP_CODE '||row_dc.PYJA_COMP_CODE||
		','||' JCAT_JOB_CODE '||row_dc.PYJA_JOB_CODE||
		','||' JCAT_PHS_CODE '||row_dc.PYJA_PHS_CODE||
		','||' JCAT_CODE '||row_dc.PYJA_CAT_CODE||
...
BEGIN
        ...
        display_status(' PYJA_PHS_CODE - checking');
        PYJA_PHS_CODE;
