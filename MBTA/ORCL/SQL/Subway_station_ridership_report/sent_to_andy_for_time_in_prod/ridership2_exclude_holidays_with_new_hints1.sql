SELECT /*+ USE_MERGE(STA,TVM,ST,MCM,SUB,SD) */  -- quest sql optimizer in test says 49 mins in prod took 1hr 49 mins
       1, 
       1, 
       sta.Name, 
 ...
 WHERE 1 = 1 
   ...
   AND (to_char(sd.creadate, 'D') not in ('1', '7') 
         or trunc(sd.creadate) not in (select * 
                                         from (
...
select to_date('05-25-2009 00:00:00', 'mm-dd-yyyy hh24:mi:ss') dateis from dual                                         )
                                        where dateis between trunc(to_date('2010-04-01-00-00-01', 'YYYY-MM-DD-HH24-MI-SS')) and trunc(to_date('2010-05-01-00-00-01', 'YYYY-MM-DD-HH24-MI-SS')))
       ) 
   AND EXISTS (SELECT 'X' 
                 FROM DeviceClass 
                WHERE DeviceClassType IN (5) 
                  AND DeviceClassID = mcm.DeviceClassID) 
   AND sd.CreaDate >= to_date('2010-04-01-00-00-01', 'YYYY-MM-DD-HH24-MI-SS') 
   AND sd.CreaDate <= to_date('2010-05-01-00-00-01', 'YYYY-MM-DD-HH24-MI-SS') 
   AND sd.PartitioningDate >= to_date('2010-04-01-00-00-01', 'YYYY-MM-DD-HH24-MI-SS') 
