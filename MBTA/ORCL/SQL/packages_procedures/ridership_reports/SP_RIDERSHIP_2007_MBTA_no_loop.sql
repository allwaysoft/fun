...
--------------------------------------------------------------------------------
  ...
  vnDays		NUMBER		(10);
	...
	dDateFirst_loop				DATE;											-- Date first for 1. partitioning section
	dDateFirst_loop				DATE;											-- Date first for 2. partitioning section
...
--
  ...
  AND (to_char(sd.creadate, ''D'') not in (''1'',''7'')
       or trunc(sd.creadate) not in (select dateis from holiday_mbta where dateis between trunc(:dDateFirst) and trunc(:dDateLast))
      ) 
';
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
--
--	377	Route Ridership by Garage and Exclude Weekend and Holidays               --MBTA
--
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
--
--	399	Time of Day Line Summary and Exclude Weekend and Holidays               --MBTA
--
--------------------------------------------------------------------------------
...
select count(1) into vndays_1                        --	count of days in 1. section
      from ( select rownum rnum
               from all_objects
              where rownum <= ceil(dDateLast_1 - dDateFirst_1) )
     where to_char( dDateFirst_1 + rnum-1, 'D' ) 
                not in ('1','7' )
     and not exists 
        ( select null from holiday_mbta where dateis =
                 trunc(dDateFirst_1 + rnum-1) );                
*/
...
/*
 select count(1) into vndays_3                        --	count of days in 3. section
      from ( select rownum rnum
               from all_objects
              where rownum <= ceil(dDateLast - dDateFirst_3))
...
--------------------------------------------------------------------------------
	...
	sp_Output	(To_Char (dDateFirst, 'YYYY-MM-DD')||', '||To_Char (dDateLast, 'YYYY-MM-DD')||', All Days: '|| To_Char(nDays) || ', Not Holidays: '|| To_Char (vnDays));
...
/*
	sp_Output	(To_Char (dDateFirst, 'YYYY-MM-DD')||', '||To_Char (dDateLast, 'YYYY-MM-DD')||', '||To_Char (vnDays));
...
----------------
    ...
    print_c := stmt_main_single||'1a'|| ' ' ||dDateFirst|| ' ' ||dDateLast|| ' ' ||dDateFirst|| ' ' ||dDateLast_1|| ' ' || ndays;
