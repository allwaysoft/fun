...
exec DBMS_STATS.SET_SCHEMA_PREFS('SYSADM', 'METHOD_OPT','FOR ALL INDEXED COLUMNS SIZE 1');
 
--SYSADM schema no histograms are necessary according to the peoplesoft red papaer.
exec dbms_stats.SET_GLOBAL_PREFS('DEGREE', 'DBMS_STATS.DEFAULT_DEGREE');

exec dbms_stats.SET_GLOBAL_PREFS('STALE_PERCENT', 1);

-- This is to make the stats run on tables, even if there is a small change in data.
