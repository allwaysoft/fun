...
select '/u01/app/oracle/admin/scripts/logs_' vpath, SYS_CONTEXT ('USERENV', 'instance_name') loc, '/alter_schema_at_login.log' ext from dual;  
...
drop trigger PS_MB_SET_DEFLT_SESSION
/

CREATE OR REPLACE TRIGGER PS_MB_SET_DEFLT_SESSION
   AFTER LOGON ON DATABASE
   WHEN (USER IN ('COMP','DEVELOPER','EKELLY','FILEBOUND','IDOSOUZA'
                 ,'KPABBA','MBTACON','MCRS','MHACHEY','OPRTN','PS','TRACS'
				 )
	    ) 
   /*
   (USER NOT IN ('SYS','SYSTEM','OUTLN','DIP','ORACLE_OCM','DBSNMP','APPQOSSYS','WMSYS','XS$NULL','EXFSYS','CTXSYS','XDB','ANONYMOUS',
                      'SI_INFORMTN_SCHEMA','MDSYS','OLAPSYS','OWBSYS_AUDIT','MDDATA','SPATIAL_WFS_ADMIN_USR','SPATIAL_CSW_ADMIN_USR',
                      'APEX_PUBLIC_USER','APEX_030200','OWBSYS','PSADMN','DBADMIN','ORDSYS','ORDDATA','ORDPLUGINS','SYSMAN','MGMT_VIEW','FLOWS_FILES'
                     )
   )
   */   
   BEGIN                                                          
