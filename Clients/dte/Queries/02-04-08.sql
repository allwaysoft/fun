...
union
select /*+ ORDERED FIRST_ROWS PARALLEL(AD, 12) PARALLEL(RMI, 12) PARALLEL(AG, 12) PARALLEL(FCH, 12) */ distinct
to_char(AD.CAD_ACCOUNT_NUMBER) "Acctno" , 
to_char(AD.CAD_CUSTOMER_NAME) "Name" , 
to_char(AD.CAD_ENERGY_TYPE) "EGB" , 
to_char(dbms_lob.instr(cad_account_xmlstring, '<profileCode>LI<',1,1)),
