...
spool \\Fileserver1\Operations\kranthiPabba\MISC\SQL\mbisgrant_schedule.lst
...
REM PY
                            REM grant select on DA.PYEMPLOYEE_TABLE to &&C_user_name;       
...
REM CADDELL CUSTOM REPORT VIEWS
    grant select on DA1.TIME_SHEET_CADVW to &&C_user_name;
    grant select on DA1.EMPLOYEE_PAY_CADVW to &&C_user_name;
    grant select on DA1.NACHA_FILE_VENDOR_CADVW to &&C_user_name;
    grant select on DA1.CHANGE_ORDER_LOG_CADVW to &&C_user_name;
    grant select on DA1.COST_TO_COMPLETE_CADVW to &&C_user_name;
    grant select on DA1.NACHA_FILE_VENDOR_CADVW to &&C_user_name;
    grant select on DA1.NACHA_FILE_FORMAT_CADVW to &&C_user_name;

    grant execute on DA1.EUL_DISCO_PKG to &&C_user_name;

                                             REM For Venodr Email Process
REM grant execute on DA.CC_UTL_PKG to &&C_user_name;
REM grant execute on da.DBK_AP_PRTCHK_CMETH to &&C_user_name;
REM grant select on DA.PAYSEL to &&C_user_name;
REM grant all on da.sysqparm to &&C_user_name;
REM grant all on da.reportcontacts to &&C_user_name;
REM grant select on da.JFAPMAST to &&C_user_name
REM grant select on da.jfapdet to &&C_user_name
REM grant select on da.payamt to &&C_user_name
                                             REM For Vendor Email Process


                                             REM For Employee Email Process
REM grant execute on DA.CC_UTL_PKG to &&C_user_name;
REM grant execute on da.dbk_stb_proc to &&C_user_name;
REM grant execute on da.dbk_py_stub to &&C_user_name;
REM grant all on da.sysqparm to &&C_user_name;
REM grant all on da.pytemptab to &&C_user_name;
REM grant all on da.jfpydet to &&C_user_name;
REM grant all on da.v_pystub_01 to &&C_user_name;
REM grant select on da.uetd_extendedde to &&C_user_name;
REM grant select on da.pycontrol to &&C_user_name;
REM grant select on da.pypayrun to &&C_user_name;
REM grant select on da.pycompayprd to &&C_user_name;
REM grant select on da.pychkloc to &&C_user_name;
REM grant select on da.pycompaygrp to &&C_user_name;
                                             REM For Employee Email Process
