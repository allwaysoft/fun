...
CUR_VAR           CUR_REF;
V_LOOP_CNT        NUMBER :=0 ;
V_CUR_VALUE       VARCHAR2(256);
FNAME             VARCHAR2(10) :='VEN_PAY';
SENDER            VARCHAR2(50) :='ORACLE.DBA@CADDELL.COM';
RECEPIENT         VARCHAR2(50) :='KRANTHI.PABBA@GMAIL.COM';
V_ERR_MSG         MAX_STRING%TYPE := '';
...
V_SQLSTMT := 'SELECT DISTINCT BPVEN_BP_CODE BP_CODE '
          || 'FROM DA.BPVENDORS '
          || 'WHERE BPVEN_BP_CODE IN ' 
...
OPEN CUR_VAR FOR V_SQLSTMT;       
  LOOP
    ...
    INSERT INTO DA1.VENDOR_DIRDEP_EMAIL(VDE_BP_CODE, VDE_EMAIL_ID, VDE_STATUS, VDE_PROCESS_DATE)
         VALUES (V_CUR_VALUE
               , 'KRANTHI.PABBA@CADDELL.COM'
               , 'SENT'
