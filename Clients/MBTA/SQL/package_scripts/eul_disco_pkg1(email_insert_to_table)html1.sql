...
V_HEADER_L10      HEADER_STRING%TYPE := 'Date    Number     Invoice Description               Amount      Amount      Amount';
V_HEADER_L11      HEADER_STRING%TYPE := '------- ---------- ---------------------------- ----------- ----------- -----------';
V_CHEADER_0      HEADER_STRING%TYPE := 'Invoice Date';
V_CHEADER_2      HEADER_STRING%TYPE := 'Invoice Number';
V_CHEADER_3      HEADER_STRING%TYPE := 'Invoice Description';
V_CHEADER_4      HEADER_STRING%TYPE := 'Invoice Amount';
V_CHEADER_5      HEADER_STRING%TYPE := 'Discount Amount';
V_CHEADER_6      HEADER_STRING%TYPE := 'Payment Amount';
...
OPEN CUR_VAR FOR V_SQLSTMT;
  LOOP
    ...
    BEGIN
      ...
      V_HEADER_TEXT := '<TABLE WIDTH="100%"> <TR> <TD ALIGN="CENTER">' || V_HEADER_L1 ||'</TD> </TR>'
                    || CRLF
                    || '<TR> <TD ALIGN="CENTER">' || V_HEADER_L2 ||'</TD> </TR>'
                    || CRLF
                    || '<TR> <TD ALIGN="CENTER">' || V_HEADER_L3 ||'</TD> </TR>'
                    || CRLF
                    || '<TR> </TR>'
                    || CRLF
                    ...
                    || '<TR> </TR> </TABLE>'
                    || CRLF
                    || '<TABLE ALIGN="CENTER"> <TR> <TD ROWSPAN="2">' || V_CHEADER_0 ||'</TD> '                   
                    || CRLF
                    || '<TD ROWSPAN="2">' || V_CHEADER_2 ||'</TD> '
                    || CRLF
                    || '<TD ROWSPAN="2">' || V_CHEADER_3 ||'</TD> '
                    || CRLF
                    || '<TD ROWSPAN="2" ALIGN="RIGHT">' || V_CHEADER_4 ||'</TD> '
                    || CRLF
                    || '<TD ROWSPAN="2" ALIGN="RIGHT">' || V_CHEADER_5 ||'</TD> '
                    || CRLF
                    || '<TD ROWSPAN="2" ALIGN="RIGHT">' || V_CHEADER_6 ||'</TD> </TR> </TABLE>' 
                    || CRLF;
...
DBMS_OUTPUT.PUT_LINE(5);
      ...
      --open the connection with the smtp server and do the handshake
        ...
        UTL_SMTP.WRITE_DATA(V_CONN, 'SUBJECT: ' || V_CUR_VALUE || '_CHECK' || UTL_TCP.CRLF);
