...
/*
V_SQLSTMT := 'SELECT JFAPM_PROC_ID || RPAD(RTRIM(JFAPM_MAST_KEY),21) JFAPM_PROC_MAST_KEY '
          || ',JFAPM_CHQ_VEN_CODE || '' - '' || JFAPM_VEN_NAME1 VENDOR'
          || ',JFAPM_CHQ_DATE, JFAPM_CHQ_NUM, C.COMP_CODE COMP_CODE '
          || 'FROM 	DA.JFAPMAST '
          || ',DA.COMPANY C '
          || ',(SELECT DISTINCT EMAIL.VDE_VEN_CODE VDE_VEN_CODE '
          ...
          || 'WHERE JFAPM_COMP_CODE = C.COMP_CODE '
          || 'AND DA.DBK_AP_PRTCHK_CMETH.F_IsVendor_IN_DistList(jFAPM_COMP_CODE,jFAPM_CHQ_VEN_CODE,0) IS NOT NULL '          
          || 'AND VEN.JFAPM_CHQ_VEN_CODE = MAIL.VDE_VEN_CODE (+) '
          || 'AND MAIL.VDE_VEN_CODE IS NULL ';
...
DBMS_OUTPUT.PUT_LINE(3);      
      ...
      SELECT BPVEN_PAY_EMAIL INTO RECEPIENT 
      FROM DA.BPVENDORS 
        ...
        LOOP
          ...
          THEN
            TEXT := --V_HEADER_TEXT
                 --||
                 '<TR> </TR>'
                 || '<TBODY> <TR> <TD>' || CUR_VEN_DET_REC.INV_DATE ||'</TD> '
