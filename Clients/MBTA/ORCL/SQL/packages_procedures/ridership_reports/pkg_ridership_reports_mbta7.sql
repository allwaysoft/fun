DROP PACKAGE MBTA.PKG_RIDERSHIP_REPORTS_MBTA;

CREATE OR REPLACE PACKAGE MBTA.pkg_ridership_reports_mbta
AS
/*********************************************************************
...
/
DROP PACKAGE BODY MBTA.PKG_RIDERSHIP_REPORTS_MBTA;

CREATE OR REPLACE PACKAGE BODY MBTA.pkg_ridership_reports_mbta
AS
/*********************************************************************************************************************************************/
/*********************************************************************************************************************************************/
...
--------------------------------------------------------------------------------
--
-- Change: 2011-07-06  MBTA KP Added the condition to exclude both Brocton and Metrowest ' AND sd.sellingrrid not in (2,3) AND '||pWhere||' 
--
--------------------------------------------------------------------------------
...
';
--------------------------------------------------------------------------------
	stmt_Group_Start			VARCHAR2	(   100)	:=							-- Group By statement start
'GROUP	BY
';
...
--------------------------------------------------------------------------------
--
--	501 SubwayRidershipByFareMediaType                       -- Rob Creedon
--
--------------------------------------------------------------------------------

	WHEN	nReportId	=	501 	THEN
--------
		stmt_Insert_List	:=	stmt_Insert_List	||
' ,number2
 ,number3
...
' AND sd.sellingrrid not in (2,3) AND '||pWhere||''    
;
...
--------------------------------------------------------------------------------
/*
if CEIL((dDateLast -dDateFirst) = dDateLast -dDateFirst) and dDateLast -dDateFirst <> 0
then
v_totaldays := CEIL(dDateLast -dDateFirst)+1;
else
...
--------------------------------------------------------------------------------
--
--	404 	 	Exits: Type and Time of Day Station Summary
--
--------------------------------------------------------------------------------
WHEN	nReportId	=	404 THEN
...
--
  ...
  1100
	...
	AND	se.CreaDate			>=      :dDateFirst                                             --to_date(:dDateFirst, ''mm/dd/yyyy hh24:mi:ss'')
	AND	se.CreaDate			<=      :dDateLast
	AND	se.PartitioningDate	>= :dPartitioningDateFirst
	AND	se.PartitioningDate	<   :dPartitioningDateLast
 ';
...
--------------------------------------------------------------------------------
--
--	405 	 	Exits: Day of the Week Station Summary
--
--------------------------------------------------------------------------------
WHEN	nReportId	=	405 THEN
...
--------------------------------------------------------------------------------
  ...
  ' AND ' || pWhere||' '
  --' AND sd.sellingrrid not in (2,3) AND '||pWhere||''   -- Had to use this condition individually to certain reports in this procedure as all of them are not using salesdetail table.
;
...
loop                                                         -- Loop to divide the given date range in to 90 day ranges(approx 6 months) temp space
