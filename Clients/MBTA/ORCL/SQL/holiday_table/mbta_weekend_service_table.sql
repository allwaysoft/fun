...
CREATE TABLE MBTA_WEEKEND_SERVICE
  (
...
comment on table mbta_weekend_service is 'Contains data for days (by hour) in which a weekend service schedule is used.  This could be weekend days or holidays.'
/
comment on column mbta_weekend_service.service_date is 'Stores weekend service schedule dates by each hour.'
/
...
select to_date('12/26/2011', 'mm/dd/yyyy' ), 'Day after Christmas Day' , 0 from dual union all
...
--where trunc(service_date)+1 not in (select trunc(service_date) from MBTA_WEEKEND_SERVICE)
/
--------------
...
/
---------------
CREATE UNIQUE INDEX PK_MBTA_WEEKEND_SERVICE_DATE ON MBTA_WEEKEND_SERVICE
(SERVICE_DATE)
TABLESPACE USER_DATA_G
/
------------
ALTER TABLE MBTA_WEEKEND_SERVICE ADD (
  CONSTRAINT CK_MBTA_WEEKWND_SERVICE_TYPE
  CHECK (SERVICE_TYPE in (1,2,3)),
  CONSTRAINT PK_MBTA_WEEKEND_SERVICE_DATE
  PRIMARY KEY
  (SERVICE_DATE)
  USING INDEX PK_MBTA_WEEKEND_SERVICE_DATE)
/
...
select count(1) from mbta_weekend_service
/