...
SELECT E.DEPTID DEPTID 
 ...
 , MAX(A.MONETARY_AMOUNT) MONETARY_AMOUNT 
 , MAX(CASE WHEN A.TRANSACTION_GROUP = '030' THEN A.MONETARY_AMOUNT ELSE 0 END) USAGE_AND_SHIPMENTS 
 , MAX(CASE WHEN A.TRANSACTION_GROUP = '041' THEN A.MONETARY_AMOUNT ELSE 0 END) CYCLE_COUNT_ADJUSTMENTS 
 , MAX(CASE WHEN A.TRANSACTION_GROUP = '050' THEN A.MONETARY_AMOUNT ELSE 0 END) USER_ADJUSTMENTS 
 , MAX(CASE WHEN A.TRANSACTION_GROUP = '051' THEN A.MONETARY_AMOUNT ELSE 0 END) INVENTORY_SCRAP 
 , MAX(CASE WHEN A.TRANSACTION_GROUP NOT IN ('030', '041', '050', '051') THEN A.MONETARY_AMOUNT ELSE 0 END) OTHER_TRANSACTIONS 
 , SUM(CASE 
 ...
 , A.FROM_DATE 
 , A.THRU_DATE 
  FROM (SELECT IA.BUSINESS_UNIT BUSINESS_UNIT
             ...
             , SF.OPRID OPRID
             , SF.RUN_CNTL_ID RUN_CNTL_ID
             , SF.FROM_DATE FROM_DATE
             , SF.THRU_DATE THRU_DATE
		  FROM PS_CM_ACCTG_LINE IA
 		  ...
 		  WHERE IA.ACCOUNTING_DT BETWEEN '2011-08-27' AND '2011-09-02'--F.FROM_DATE AND F.THRU_DATE  		   
		    ...
		    AND SF.OPRID = 'MBBATCH'--G.OPRID 
		    AND SF.RUN_CNTL_ID = 'MBINOP02_225016'--G.RUN_CNTL_ID
		    AND IA.FUND_CODE = '05'  
 ...
 WHERE A.BUSINESS_UNIT = B.BUSINESS_UNIT 
   ...
   AND C.SETID = 'SHARE' 
 GROUP BY C.INV_ITEM_ID , C.DESCR , E.DEPTID , E.DESCR , A.ACCOUNTING_DT , A.TRANSACTION_GROUP , A.OPRID , A.RUN_CNTL_ID , A.FROM_DATE , A.THRU_DATE
...
--  , PS_CM_ACCTG_LINE A      
  ...
  --, PS_MB_RUNCTL_IN F 
  --, PS_MB_RUNCTL_IN02 G 
 WHERE 1=1--A.ACCOUNTING_DT BETWEEN IA.FROM_DATE AND IA.THRU_DATE --'2011-08-27' AND '2011-09-02'--F.FROM_DATE AND F.THRU_DATE
--   AND A.DEPTID = IA.DEPTID --'225016'--G.DEPTID 
--   AND F.OPRID = 'MBBATCH'--G.OPRID 
 --  AND F.RUN_CNTL_ID = 'MBINOP02_225016'--G.RUN_CNTL_ID 
--   AND A.FUND_CODE = '05'    
--   AND A.ACCOUNT NOT IN ('1510000' , '1520000' , '1590495') 
   --AND A.BUSINESS_UNIT = IA.BUSINESS_UNIT 
   --AND A.INV_ITEM_ID = IA.INV_ITEM_ID   
   ...
   AND A.DEPTID = E.DEPTID 
   AND C.SETID = 'SHARE' 
 GROUP BY C.INV_ITEM_ID , C.DESCR ,E.DEPTID , E.DESCR 
          --, A.ACCOUNTING_DT , A.TRANSACTION_GROUP 
 , A.OPRID 
 , A.RUN_CNTL_ID 
 , A.FROM_DATE 
 , A.THRU_DATE 
