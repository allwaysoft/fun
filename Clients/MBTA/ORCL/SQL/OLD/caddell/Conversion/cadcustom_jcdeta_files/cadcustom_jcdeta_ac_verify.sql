...
dbms_output.put_line(chr(9));
	...
	values(2,'JCDETAIL', '---SUCCESSFUL---No Errors while running the script CADCUSTOM_VERIFY.SQL');
...
then 
	...
	values(2, 'JCDETAIL', -20100, 'No records in table: '||v_dc_tab_name);
	commit;
dbms_output.put_line(chr(9));
...
dbms_output.put_line(chr(9));
	...
	select 2,'JCDETAIL',-2101, dcerr_err_type, dcerr_column_name, dcerr_description 
        from dc_error
...
then  
	v_err_strng:=SQLERRM;
        INSERT into da1.errors(ers_order, ERS_TABLE_NAME, ERS_ROWNUM, ers_description) 
	values(2, 'JCDETAIL', -20102,'Error: '||v_err_strng||'.');
	commit;
dbms_output.put_line(chr(9));
DBMS_OUTPUT.PUT_LINE('Error type is inserted into table: DA1.ERRORS');
