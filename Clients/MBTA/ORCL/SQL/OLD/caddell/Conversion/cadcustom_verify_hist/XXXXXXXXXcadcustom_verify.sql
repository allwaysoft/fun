...
v_tabs_rem := 'BPARTNERS, BPADDRESSES, BABANK, BABRANCH, BABANKACCT, BPBANKS, PYCOMPAYGRP, '
            ||'PYTRADES, PYWCBCODE, PYWCBRATE, PYEMPLOYEE_TABLE, PYEMPSECGRP, '
            ||'PYEMPSECGRPEMP, PYEMPSALSPL, PYCHKLOC, '
...
/*
-- PYEMPSECGRP is directly inserted by the script cadcustom_dctables.bat in to 
-- the main table no verify present for this table.
...
select count(1) into v_cnt from da.PYEMPSECGRP;
v_dc_tab_name := 'PYEMPSECGRP';
...
raise no_data_found;
    ...
    select count(1) into v_cnt 
    from PYEMPSECGRP;
    v_cnt_tab := v_cnt||' into PYEMPSECGRP';
--    DBMS_OUTPUT.PUT_LINE('Insert into PYEMPSECGRP Successful');
--    DBMS_OUTPUT.PUT_LINE(v_cnt_tab ||'records inserted.');

end if;

v_tabs_rem := replace(v_tabs_rem,'PYEMPSECGRP, '); 
...
v_tabs_rem := replace(v_tabs_rem,'SCMAST, ');
	...
	commit;
	v_msg1 := v_msg1||'----SCMAST table updated based on PMPROJECT_TABLE.';
