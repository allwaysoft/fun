...
--DBMS_OUTPUT.PUT_LINE(4);
      ...
      OPEN CUR_VEN_DET(V_CUR_VALUE);
        LOOP
          ...
          THEN
            TEXT := '<TR> </TR>' -- Required for correct output in outlook
                 || '<TBODY> <TR> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> </TR> '
        ...
        END LOOP;
            ...
            TEXT := TEXT
                 || '<TR> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> </TR>'
                 || '<TR> <TD>' || '' ||'</TD> '
                 || '<TD>' || '' ||'</TD> '
...
V_SQLSTMT := 'SELECT DISTINCT VDE_VEN_CODE || '' '' || '
          ...
          || 'AND VDE_STATUS NOT IN (''SENT'', ''NOTIFIED'') ';
TEXT := '';
V_LOOP_CNT := 0;
...
OPEN CUR_VAR FOR V_SQLSTMT;
  ...
  V_SQLSTMT := 'UPDATE DA1.VENDOR_DIRDEP_EMAIL '
            || 'SET VDE_STATUS = ''NOTIFIED'' '
            || 'WHERE TRUNC(VDE_PROCESS_DATE) = TRUNC(SYSDATE) '
            || 'AND VDE_STATUS = ''NOT SENT'' ';
...
WHERE STB.C_ORDER_BY = 1
  ...
  AND STB.EMP_CODE = EMP.EMP_NO
  AND UPPER(EXT.PYEMAILDD) = 'Y'
  AND EMP.EMP_EMAIL_ADDRESS IS NOT NULL
...
WHERE TRUNC(EDE_PROCESS_DATE) <= TRUNC(SYSDATE)-1;
COMMIT;

SELECT COMP_NAME
     , COMP_ADD3 || ', ' || COMP_REGION_CODE || ', ' || COMP_POSTAL_CODE
...
DBMS_OUTPUT.PUT_LINE(3);
      ...
      V_HEADER_TEXT := '<TABLE WIDTH="100%"> <TR> <TH ALIGN="CENTER">' || V_HEADER_L1 ||'</TH> </TR>'
                    ...
                    || '<TH ALIGN="LEFT">' || V_EMPHEADER_3 ||'</TH> ' --BGCOLOR="GRAY"
                    || '<TH ALIGN="LEFT">' || V_EMPHEADER_4  ||'</TH> </TR> '
                    
                    || '<TR> <TH ALIGN="LEFT">' || V_EMPHEADER_2 ||'</TH> ' --BGCOLOR="GRAY"
                    || '<TH ALIGN="LEFT">' || V_EMPHEADER_6 ||'</TH> '
                    || '<TH ALIGN="LEFT">' || V_EMPHEADER_7 ||'</TH> </TR>' 
                    
                    || '<TR> <TH ALIGN="LEFT">' || V_EMPHEADER_5 ||'</TH> ' --BGCOLOR="GRAY"
                    || '<TH ALIGN="LEFT">' || '' ||'</TH> '
                    || '<TH ALIGN="LEFT">' || '' ||'</TH> </TR> </THEAD> </TABLE>'
                    || '<TABLE WIDTH="100%"> <TR> <HR NOSHADE SIZE=1 COLOR=BLACK > </TR> </TABLE>'
...
DBMS_OUTPUT.PUT_LINE(44);
          ...
          THEN
            TEXT := '<TR> </TR>' -- Required for correct output in outlook
                 || '<TBODY> <TR> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> </TR> '
                 || '<TR> <TD>' || CUR_EMP_DET_REC.EARN_DESC ||'</TD> '
                 || '<TD ALIGN="RIGHT">' || TO_CHAR(CUR_EMP_DET_REC.EARN_AMT, '999,999,990.99') ||'</TD> '
                 || '<TD ALIGN="RIGHT">' || TO_CHAR(CUR_EMP_DET_REC.EARN_HRS, '999,999,990.99') ||'</TD> '
                 || '<TD ALIGN="RIGHT">' || TO_CHAR(CUR_EMP_DET_REC.EARN_YTD, '999,999,990.99') ||'</TD> '
                 || '<TD>' || CUR_EMP_DET_REC.DED_DESC ||'</TD> '
                 || '<TD ALIGN="RIGHT">' || NVL(TO_CHAR(CUR_EMP_DET_REC.DED_AMT, '999,999,990.99'), 0.00) ||'</TD> '
                 || '<TD ALIGN="RIGHT">' || NVL(TO_CHAR(CUR_EMP_DET_REC.DED_YTD, '999,999,990.99'), 0.00) ||'</TD> </TR>';
            V_LOOP_CNT := V_LOOP_CNT+1;
          ELSE
            TEXT := TEXT
                 || '<TR> <TD>' || CUR_EMP_DET_REC.EARN_DESC ||'</TD> '
...
/*
            TEXT := TEXT
--                 || '<TR> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD></TR>'
                 || '<TR> <TD ALIGN="RIGHT">' || 'Gross Pay:' ||'</TD> '
                 || '<TD ALIGN="RIGHT">' || NVL(TO_CHAR(V_GROSS, '999,999,990.99'), 0.00) ||'</TD> '
                 || '<TD>' || '' ||'</TD> '
                 || '<TD ALIGN="RIGHT">' || V_YTD_GROSS ||'</TD> '
                 || '<TD>' || '' ||'</TD> ' 
                 || '<TD>' || '' ||'</TD> ' 
                 || '<TD>' || '' ||'</TD> </TR>';
*/
...
V_HEADER_FOOT := '<TABLE WIDTH= "70%" ALIGN="LEFT"> <THEAD> <TR> <TH ROWSPAN="1" ALIGN="LEFT">' || V_BNKHEADER_1 ||'</TH> '
      ...
      OPEN CUR_BANK_DET(V_CUR_VALUE);
        LOOP
          ...
          IF V_LOOP_CNT1 = 0
          THEN                 
V_FOTTER_TEXT := V_HEADER_FOOT 
              || '<TR> </TR>' -- Required for correct output in outlook
              || '<TBODY> <TR> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> <TD> <HR NOSHADE SIZE=1 COLOR=BLACK > </TD> </TR> '
              ...
              || '<TD ALIGN="RIGHT">' || NVL(TO_CHAR(CUR_BANK_DET_REC.AMT, '999,999,990.99'), 0.00) ||'</TD> </TR>';
              V_LOOP_CNT1 := V_LOOP_CNT1+1;
          ELSE
V_FOTTER_TEXT := V_FOTTER_TEXT
              || '<TR> <TD ALIGN="LEFT"> ' || CUR_BANK_DET_REC.ROUTING ||'</TD> '
              || '<TD ALIGN="LEFT">' || CUR_BANK_DET_REC.ACCT ||'</TD> '
              || '<TD ALIGN="LEFT">' || CUR_BANK_DET_REC.ACCTYPE ||'</TD> '
              || '<TD ALIGN="RIGHT">' || NVL(TO_CHAR(CUR_BANK_DET_REC.AMT, '999,999,990.99'), 0.00) ||'</TD> </TR>';
          END IF;
       END LOOP;        
V_FOTTER_TEXT := V_FOTTER_TEXT || ' </TBODY> </TABLE>';
      CLOSE CUR_BANK_DET;              
...
DBMS_OUTPUT.PUT_LINE(5);
      ...
      --open the connection with the smtp server and do the handshake
        ...
        UTL_SMTP.WRITE_DATA(V_CONN, 'FROM: ' || P_SENDER || UTL_TCP.CRLF);
        UTL_SMTP.WRITE_DATA(V_CONN, 'TO: '   || RECEPIENT || UTL_TCP.CRLF);
        UTL_SMTP.WRITE_DATA(V_CONN, 'SUBJECT: Confidential Notification of Direct Payroll Deposit' || UTL_TCP.CRLF);
        UTL_SMTP.WRITE_DATA(V_CONN, 'MIME-VERSION: 1.0' || UTL_TCP.CRLF);
        UTL_SMTP.WRITE_DATA(V_CONN, 'CONTENT-TYPE: TEXT/HTML' || UTL_TCP.CRLF);
        UTL_SMTP.WRITE_DATA(V_CONN, 'REPLY-TO: ' || P_REPLY_TO || UTL_TCP.CRLF);
        UTL_SMTP.WRITE_DATA(V_CONN, UTL_TCP.CRLF);
...
V_SQLSTMT := 'SELECT DISTINCT EDE_EMP_CODE || '' '' || '
          ...
          || 'AND EDE_STATUS NOT IN (''SENT'', ''NOTIFIED'') ';
TEXT := '';
V_LOOP_CNT := 0;
...
OPEN CUR_VAR FOR V_SQLSTMT;
  ...
  V_SQLSTMT := 'UPDATE DA1.EMP_DIRDEP_EMAIL '
            || 'SET EDE_STATUS = ''NOTIFIED'' '
            || 'WHERE TRUNC(EDE_PROCESS_DATE) = TRUNC(SYSDATE) '
            || 'AND EDE_STATUS = ''NOT SENT'' ';            
  EXECUTE IMMEDIATE V_SQLSTMT;
  COMMIT;  
